...
# TODO: The NCHG script still just runs on one core per input file, so that is the major bottleneck, but this at least halved the run time.
#       Also clean up code in general, make os.ispath and stay consistent between methods, so it's more readable...
...
class SetDirectories:
    ...
    def set_threads(threads):
        SetDirectories.threads = int(threads)
...
class Pipeline:
    ...
    def make_bedpe(bed_file, matrix_file):
        ...
        try:
            ...
            print(f"Error processing file: {bed_file, matrix_file}, PID: {os.getpid()}, TID: {tid}")
            raise
    ...
    def input_to_make_bedpe(grouped_files):
        ...
        for key, val in grouped_files.items():
            ...
            # TODO: Parallelize this (can't and doesn't make sense until I separate the text formating from the processing)
            bedfile = val[0]
            matrixfile = val[1]
    ...
    def remove_blacklisted_regions(bedpe_file):
        ...
        try:
            ...
            tid = threading.get_ident()
            print(f"Error processing file: {bedpe_file}, PID: {os.getpid()}, TID: {tid}")
            raise
